...
1. [What's included
2. [Ready-made APKs
3. [Install on the tablet (Windows + ADB)
4. [First-run setup
5. [Making the on-screen keyboard smaller
6. [The hidden admin menu
7. [Kiosk mode
8. [Device Owner (true Lock Task)
9. [Removing Device Owner status safely
10. [Recovering the device if kiosk mode causes problems
11.  [Auto-start after reboot
12.  [Website updates & the refresh action
13.  [Building the APK yourself
14.  [Configuration reference
15.  [Replacing the launcher icon / splash logo
16.  [What the WebView enables (feature map)
17.  [Security notes
18.  [Test checklist for the showroom tablet
19.  [Troubleshooting
20.  [How the shipped APKs were built
...
|---|---|---|---|
| `apk/BenAmorPriceChecker-1.0.2-release.apk` | ≈ 78 KB | `keystore/benamor-release.jks` | **production — install this one** |
| `apk/BenAmorPriceChecker-1.0.2-debug.apk` | ≈ 78 KB | debug key | quick testing |

`apk/latest.json` is the self-update manifest the app reads from the `main`
branch on GitHub (regenerated by every build — see
[Updating the APK from the tablet](#updating-the-apk-from-the-tablet-self-update)).
...
```bat
adb install -r "%USERPROFILE%\Downloads\BenAmorPriceChecker-1.0.2-release.apk"
```
...
- **If Android refuses** (`INSTALL_FAILED_UPDATE_INCOMPATIBLE` — different
  ...
  adb install "android\apk\BenAmorPriceChecker-1.0.2-release.apk"
  ```
...
### Updating the APK from the tablet (self-update)

From **v1.0.2** onwards the app can update **itself** — no PC needed. The one
last `adb install` is v1.0.2 itself; every later version installs from the
admin menu:

1. Every build regenerates `android/apk/latest.json` (version + APK
   filename). Commit it and **merge the branch to `main`** — the app reads
   this file from the project's `main` branch on GitHub
   (`Config.UPDATE_JSON_URLS`: raw.githubusercontent.com primary,
   github.io fallback).
2. On the tablet: admin menu → **تحديث تطبيق الأندرويد (APK)**.
   - No newer version → the app says you are on the latest one.
   - Newer version → confirm dialog → the app downloads the release APK
     over HTTPS into its private cache and hands it to the Android system
     installer (PackageInstaller).
3. What happens next depends on the tablet's mode:
   - **Device Owner (true kiosk):** fully silent — the new APK installs in
     the background and the app relaunches itself (a few seconds of black
     screen).
   - **Normal / non-owner kiosk:** the app stops screen pinning first, then
     Android shows the standard **"Install?"** dialog — the admin taps
     **Install** once. The *first time only*, Android also asks for the
     "install unknown apps" permission — the app opens the correct Settings
     screen automatically; toggle **Allow from this source**, then run the
     update again. A relaunch safety net fires if the dialog is dismissed.

Notes:

- **Signature rule:** Android installs an update only if it is signed with
  the same key as the installed app. A *debug* APK installed on the tablet
  can never self-update to a *release* build (or vice-versa) — the app
  detects this and tells you; fix it with a one-time `adb uninstall` +
  `adb install` of the matching variant (see above).
- The self-update downloads only from the project's own GitHub URLs over
  HTTPS, and the installer additionally verifies the APK's package name
  before anything is committed.
- `versionCode` (not the visible version name) decides what counts as
  "newer" — always bump it (see [Version bumps](#version-bumps)).
...
| تحديث تطبيق الأندرويد (APK) | **Update the Android app itself** — checks GitHub (`main` branch) for a newer APK, downloads and installs it, then relaunches (see [Updating the APK from the tablet](#updating-the-apk-from-the-tablet-self-update)) |
...
> This section is about the **website**. The **APK itself** is updated from
> the admin menu → **تحديث تطبيق الأندرويد (APK)** — see
> [Updating the APK from the tablet](#updating-the-apk-from-the-tablet-self-update).
...
> why the APK is ~80 KB, builds anywhere, and has the smallest possible
...
variables at the top of `build-apk.sh`, then rebuild — the script also
regenerates `apk/latest.json` for the in-app self-update. (Higher
`versionCode` is required for updates over an existing install, whether by
`adb install -r` or in-app.) After committing, **merge to `main`** so the
tablet's self-update can see the new version.
...
- **No analytics, no ads, no Play Services** — permissions are exactly:
  INTERNET, ACCESS_NETWORK_STATE, RECEIVE_BOOT_COMPLETED, plus
  **`REQUEST_INSTALL_PACKAGES` (v1.0.2)** so the app can update itself.
  The installer is only ever fed the APK downloaded from the project's own
  GitHub URLs (package name verified first), and Android still enforces the
  same-signature rule on top.
...
| 17 | APK self-update (after merging to `main`) | admin → تحديث تطبيق الأندرويد (APK) → newer version found → install → app relaunches on the new version |
...
| Self-update says it cannot reach update info | `latest.json` is not on the `main` branch yet — merge/push, wait ~5 min (raw/Pages cache), retry. |
| Self-update fails with a signature/install error | Installed app is signed with the other key (debug vs release) — one-time `adb uninstall` + `adb install` of the matching variant. |
| "Install?" dialog never appears (non-owner) | Grant *install unknown apps* for the app — the self-update opens that Settings page automatically when needed; toggle Allow, retry. |
